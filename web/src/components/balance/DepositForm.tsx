    ...
    try {
      ...
      if (restored.api_key) {
        ...
        if (localKey && localKey !== restored.api_key) {
          // Old key from credits purchase — archive it, use server key
    ...
    try {
      ...
      for (let i = 0; i < 120; i++) {
        ...
        if (status.status === "success") {
          ...
          setStep("done"); setSolanaIntent(null); onDeposited?.(); return;
        }
        if (status.status === "failed" || status.status === "expired") throw new Error(`Bridge ${status.status}`);
      }
      throw new Error("Bridge timeout");
    } catch (e: any) {
      setError(e?.message || "Bridge failed"); setStep("error"); setSolanaIntent(null);
    }
  }
      ...
      {step === "waiting_send" && solanaIntent && (
        <div className="bg-purple-500/[0.08] border border-purple-500/20 rounded-xl p-4 space-y-3">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => { setStep("idle"); setSolanaIntent(null); }} className="px-4 py-2.5 rounded-xl text-sm text-slate-400 bg-white/[0.04] border border-white/[0.08] hover:bg-white/[0.08] transition-all">
              Cancel
            </button>
          </div>
        </div>
